...
Below are instructions to set this up for the PostgreSQL database, under Ubuntu (tested with version 12.04).

.. note::

  Perform the following operation after having set up AiiDA.
  Only then will the ``.aiida`` folder (and the files within) be created.
  You can usually find it here: ``~/.aiida``.
...
.. note::
  ...
  Do not forget to put this file in ``~/`` (your home-directory) and to name it ``.pgpass``.
...
In order to retrieve the database from a backup, you have first to create an empty database following the instructions described above in "Setup instructions: PostgreSQL" except the ``verdi install`` phase.
Once you have created your empty database with the same names of the backuped one, type the following command::
...
.. note:: If you are experiencing memory problems and cannot enter the postgres shell, you can look directly into the file ``/etc/postgresql/9.1/main/postgresql.conf`` and check out the line defining the variable ``data_directory``.
...
Make sure the permissions, owner, and group are the same in the old and new directory (including all levels above the ``DESTINATION_DIRECTORY``).
...
Then you can change the postgres configuration file, which should typically be located at ``/etc/postgresql/9.1/main/postgresql.conf``.
...
* The destination folder of the backup.
  ...
  By default it is a folder inside the aforementioned ``backup`` folder (e.g. ``/home/aiida_user/.aiida/backup/backup_dest``).
...
A template backup configuration file (``backup_info.json.tmpl``) will be copied into the backup folder.
You can set the backup variables by yourself after renaming the template file to ``backup_info.json``, or you can answer the questions asked by the script, and then ``backup_info.json`` will be created based on you answers.
...
* ``oldest_object_backedup`` (timestamp or null):
  ...
  Then the script will search the creation date of the oldest Node object in the database and it will start the backup from that date.
  E.g. ``"oldest_object_backedup": "2015-07-20 11:13:08.145804+02:00"``

* ``end_date_of_backup``:
  If set, the backup script will backup files that have a modification date until the value specified by this variable.
  If not set, the ending of the backup will be set by the following variable (``days_to_backup``), which specifies how many days to backup from the start of the backup.
