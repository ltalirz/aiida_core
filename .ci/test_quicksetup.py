...
class ProfileManageTestCase(unittest.TestCase):
    ...
    def test_quicksetup(self):
        ...
        result = self.runner.invoke(quicksetup, [
            '--profile=giuseppe-{}'.format(self.backend), '--db-backend={}'.format(self.backend),
            '--email=giuseppe.verdi@ope.ra', '--first-name=Giuseppe', '--last-name=Verdi', '--institution=Scala',
            '--db-name=aiida_giuseppe_{}'.format(self.backend), '--repository=aiida_giuseppe_{}'.format(
                self.backend), '--non-interactive', '--db-port={}'.format(self.fixture_manager.db_port)
        ])
    ...
    def test_delete(self):
        ...
        result = self.runner.invoke(profile_delete, ['--force', profile_list[1]])
        self.assertIsNone(result.exception, result.output)

        result = self.runner.invoke(profile_list)
        self.assertIsNone(result.exception, result.output)

        self.assertNotIn(profile_list[1], result.output)
        self.assertIsNone(result.exception, result.output)

        # Delete multiple profiles
        result = self.runner.invoke(profile_delete, ['--force', profile_list[2], profile_list[3]])
        self.assertIsNone(result.exception, result.output)

        result = self.runner.invoke(profile_list)
        self.assertIsNone(result.exception, result.output)
        self.assertNotIn(profile_list[2], result.output)
        self.assertNotIn(profile_list[3], result.output)
        self.assertIsNone(result.exception, result.output)
